...
- **CI** (`.github/workflows/ci.yml`) runs on every pull request to `main`,
  ...
  **only** required status checks on `main`. The one other thing that reports
  on a PR is advisory: CodeQL's default-setup code scanning ("CodeQL", "Analyze
  ...
  that couldn't run hasn't analyzed anything.
  The required two:
